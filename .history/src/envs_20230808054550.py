...
class RealDatasetEnvironment:
    ...
    def generate_reward(self):
        self.t += 1
        self.reward = self.reward_gen.generate(self.t)
    ...
    def get_feedback(self, arm_idx, ctx=None):
        if ctx is None:
            ctx = self.ctx
        mean = (self.label == arm_idx).astype(float)
        noise = self.noise_gen.generate(ctx, arm_idx)
        max_rew = 1.0
...
class Feedback:
    ...
    def __init__(self, ctx, arm_idx, mean_rew, noise, max_rew,oracle_rew = Null):
        self.ctx = ctx
...
class RealDatasetContextGenerator(ContextGenerator):  # class inheritance
    ...
    def generate(self, t):
        return Context(t, self.context.iloc[t])

class RealDatasetRewardGenerator(ContextGenerator):  # class inheritance
    def __init__(self, d, reward):
        self.k = 2
        self.d = d
        self.reward = reward

    def generate(self, t):
        return Context(t, self.reward.iloc[t])

...
def emp_scenario(
    ...
    scaler = MinMaxScaler()
    features = scaler.fit_transform(datadf.iloc[:, :-1])
    labels = datadf.iloc[:, -1]

    if k != 2:
        print('*** Warning ***, k should be 2 for classification problems')
    ctx_gen = RealDatasetContextGenerator(d, features)
    noise_gen = NoiseGenerator.gaussian_noise(noise_sd, state=state_factory())
    reward_gen = RealDatasetLabelGenerator(d,labels)
    env = RealDatasetEnvironment(ctx_gen, reward_gen, noise_gen)
    param = state_factory().normal(prior_mu, prior_var ** 0.5, 2*d)

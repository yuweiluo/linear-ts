...
def run_experiments_k(
    ...
    timestr = time.strftime("%Y%m%d-%H%M%S")
    output_name = f"{timestr}-{n}-{d_min}-{d_max}-{d_gap}-{k}-{t}-{sim}-{prior_mu}-{prior_sd}-{noise_sd}-{thin_thresh}-{const_infl}-{gamma}"
    outputs.to_csv(f"my_outputs/all-{output_name}.csv", index=False)

    outputs_last.to_csv(f"my_outputs/all-last-{output_name}.csv", index=False)

    figure_folder_name = f"figures-{output_name}"
    ...
    for name, metric in metrics.items():
        ...
        plt.legend()
        plt.savefig(
            f"{figure_folder_name}/{'last_k'}-{name}-{output_name}.pdf")

    plot_statistics(figure_folder_name)
...
# PYTHONPATH=src python -m experiments_2 -sim 2 -k 0 -d_min 30 -d_max 60 -d_gap 50 -t 1000 -pm 0 -psd 1000 -nsd 1 -n 10 -gamma 0.1 -mode 0
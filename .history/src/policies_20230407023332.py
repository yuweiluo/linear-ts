...
class GradientDirectedWorthFunction(ProductWorthFunction):
    ...
    def update(self):
        ...
        self.compensator = (
            ...
            basis.T @ (np.multiply(1/scale[:, np.newaxis] ** 0.5, rand)) * self.noise_sd
        )
...
class PureThinnessWorthFunction(ProductWorthFunction):
    ...
    def update(self):
        ...
        self.compensator = (
            #self.inflation(self.summary) * basis.T @ (rand / scale ** 0.5)
            basis.T @ (rand / scale ** 0.5)* self.noise_sd
        )
...
class TsWorthFunction(ProductWorthFunction):
    ...
    def update(self):
        ...
        self.compensator = (
            self.inflation(self.summary) * basis.T @ (rand / scale ** 0.5)* self.noise_sd
        )

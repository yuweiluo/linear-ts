...
def run_experiments_d(
    ...
    for d in d_list:
        ...
        for k in k_list:
            ...
            file_name_suffix = f"-{k}-{n}-{d}-{k}-{t}-{sim}-{prior_mu}-{prior_sd}-{noise_sd}-{thin_thresh}-{const_infl}.jpg"
            ...
            for name, metric in metrics.items():
                ...
                plt.legend()
                plt.savefig(f"{figure_folder_name}/{name}-{k}-{n}-{d}-{k}-{t}-{sim}-{prior_mu}-{prior_sd}-{noise_sd}-{thin_thresh}-{const_infl}.jpg", dpi = 600)

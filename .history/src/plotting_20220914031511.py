...
def plot_sample_path( df, x_name,  y_name, x_axis, y_axis,index_name_1, index_name_2,  save_path , normalized):
    ...
    for index_1 in index_list_1:
        if index_1 >= 0:
            ...
            plt.ylabel(y_axis)
            plt.legend(loc = 'best', prop={'size': 6})
            plt.savefig(index_name_1+'_'+save_path+'_'+str(index_1), dpi = 600)


def plot_statistics(output_folder_name, output_name, figure_folder):
    ...
    for method in method_list:
        save_path = f'{figure_folder}/{method}/'
        isExist = os.path.exists(save_path)
        if not isExist:
            os.makedirs(save_path)

        df = pd.read_csv(f"{output_folder_name}/all-last-{output_name}.csv")
        plot_last_iter(df,'d' ,method,'d','y','k',save_path+output_name+'-1.jpg' )

        df = pd.read_csv(f"{output_folder_name}/all-{output_name}.csv")
        plot_sample_path(df, 'd', method, 't', 'y', 'd', 'k', f'{save_path}-{output_name}-2.jpg', 1)

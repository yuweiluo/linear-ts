...
###########Yuwei##########################
    ...
    def update_metrics(self, feedback: Feedback):
        ...
        if zeta_square >= 0 and theta_norm_lower_bound>0:
            ...
            self.x_XXnorm_min  =  zeta / theta_norm_upper_bound_cheat
        else:
        ...
        if self.tt == self.t:
            ...
            print(f"beta = {beta}, zeta_square = {zeta_square}, self.summary.lambda_ = { self.noise_sd * np.sqrt(2 * np.log((self.summary.lambda_ + self.t)**(self.d/2)* self.summary.lambda_ ** ( self.d/2)/ self.delta  ))_}")

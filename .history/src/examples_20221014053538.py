...
def russo_scenario(state_factory, k=1, d=100, t=1000, prior_var=2.0, arm_bound=0.1, prior_mu=0.0, noise_sd=1.0, thin_thresh=2.0, const_infl=5.0, radius=1.0):
    ...
    if k == np.inf: 
        ...
        algs = {"TS-Bayes": Roful.ts(d, prior_var=prior_var, state=state_factory(), inflation=1.0, param=param, noise_sd = noise_sd, t = t, radius = radius, delta = 1/t ),}
...
def russo_scenario_inv(state_factory, k=1, d=100, t=1000, prior_var=2.0, arm_bound=0.1, prior_mu=0.0, noise_sd=1.0, thin_thresh=2.0, const_infl=5.0, radius=1.0):

    param = state_factory().normal(prior_mu, prior_var**0.5, d)
    param = radius * param / np.linalg.norm(param, ord=2)

    #radius = prior_var**0.5 * np.sqrt(d)
    ...
    env = Environment(param, ctx_gen, noise_gen)
    if k == np.inf:
        algs = {"TS-Bayes": Roful.invts(d, prior_var=prior_var, state=state_factory(), inflation=1.0, param=param, noise_sd = noise_sd, t = t, radius = radius ),
        }
    else:
        algs = {"TS-Bayes": Roful.invts(d, prior_var=prior_var, state=state_factory(), inflation=1.0, param=param, noise_sd = noise_sd, t = t, radius = radius ),#"greedy": Roful.greedy(d, prior_var=prior_var, param=param, noise_sd = noise_sd, t = t, radius = radius ),}
        }
    for i in range(t):

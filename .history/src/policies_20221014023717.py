...
class Roful(Policy):
    ...
    @staticmethod
    # added param as input to calculate error
    def spects(d, prior_var, inflation=1.0, state=None, param=None, noise_sd = None, t = None, radius = None, delta = None, alpha = None):
        if isinstance(inflation, float):
            inflation = Roful.const_inflation(inflation)

        return Roful(d, prior_var, SgTsWorthFunction(inflation, alpha = alpha, state=state), param, noise_sd = noise_sd, t = t, , radius = radius, delta = delta, inflation = inflation)
...
class SgTsWorthFunction(ProductWorthFunction):
    ...
    def __init__(self, inflation, alpha, state=npr):
        self.inflation = inflation
        self.state = state

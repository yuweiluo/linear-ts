...
class DataSummary:
    ...
    def calculate_alpha(self):
        ...
        if theta_hat_Vt_norm_sqr >= self.radius_det**2:
            
            zeta = (theta_hat_Vt_norm_sqr - self.radius_det**2)**0.5
            #theta_norm_lower_bound = npl.norm(theta_hat) - beta/np.sqrt()
            theta_norm_upper_bound = np.maximum( npl.norm(theta_hat - self.radius_det/np.sqrt(lambda_min) * self.summary.basis[self.summary.d - 1]),npl.norm(theta_hat + self.radius_det/np.sqrt(lambda_min) * self.summary.basis[self.summary.d - 1]))
            theta_norm_upper_bound = npl.norm(theta_hat) + self.radius_det/np.sqrt(lambda_min)

...
###########Yuwei##########################
    ...
    def update_metrics(self, feedback: Feedback):
        ...
        self.x_XXinvnorm_min =  1 / self.x_XXnorm_max 
        ...
        if zeta_square >= 0 and theta_norm_lower_bound>0:
            opt_XX_inv_norm,opt_weight = self.solve_LP(self.summary.scale, self.x_XXnorm_min**2, self.x_XXnorm_max**2)
            self.approx_alpha.append(np.sqrt(self.x_XXinvnorm_min/opt_XX_inv_norm))
            theta_opt = self.summary.basis  @  np.sqrt(np.array(opt_weight))
            
            print(np.sqrt(theta_opt.T @ xx_inv @ theta_opt/ npl.norm(theta_opt)**2))
        ...
        if self.tt == self.t:
            ...
            print(f"opt_XX_inv_norm = {np.sqrt(theta_opt.T @ xx_inv @ theta_opt/ npl.norm(theta_opt)**2)}, self.x_XXnorm_min= {self.x_XXnorm_min}, self.x_XXnorm_max = {self.x_XXnorm_max},1/opt_XX_inv_norm = {1/opt_XX_inv_norm} ")

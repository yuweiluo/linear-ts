...
def example1_scenario(
        state_factory, k=3, d=300, t=1000, prior_var=1, prior_mu=0.0, noise_sd=2.0,
        thin_thresh=2.0,
        const_infl=5.0, radius = 1, delta = 0.1 
):
    param = state_factory().normal(prior_mu, prior_var ** 0.5, d)
    radius  = np.linalg.norm(param)+1e-3
    ...
    def radius_SG():
        ...
        #return l* np.sqrt(self.summary.lambda_) + self.noise_sd * np.sqrt(2 * np.log((self.summary.lambda_ + self.t)**(self.d/2)* self.summary.lambda_ ** ( - self.d/2)/ self.delta  ))
    algs = {
        ...
        'oful': Roful.oful(d, alpha=alpha, radius_SG=radius_SG, param=param, noise_sd = noise_sd, t = t, radius = radius, delta = 1/t ),
    }

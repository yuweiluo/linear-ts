...
class DataSummary:
    ...
    def calculate_alpha(self):
            ...
            opt_x_XXinvnorm_sqr_RLS, _ = self.calculate_x_XXinvnorm_sqr_range(self.inflation(self))
    ...
    def calculate_x_XXinvnorm_sqr_range(self, inflation):
        ...
        self.beta = self.inflation(self) * self.radius_det()
        if theta_hat_Vt_norm_sqr >= self.beta**2:
            ...
            theta_norm_upper_bound = np.maximum( npl.norm(theta_hat - self.beta/np.sqrt(lambda_min) * self.basis[self.d - 1]),npl.norm(theta_hat + self.beta/np.sqrt(lambda_min) * self.basis[self.d - 1]))

...
def run_experiments_gamma(
    ...
    timestr = time.strftime("%Y%m%d-%H%M%S")
    output_name = f"gammas-t-{t}-prior_sd-{prior_sd}-noise_sd-{noise_sd}-gamma-{gamma}-radius-{radius}-sim-{sim}-k-{k}-prior_mu-{prior_mu}-thin_thresh-{thin_thresh}-const_infl-{const_infl}-n-{n}-gammas-{gamma_min}-{gamma_max}-{gamma_gap}-time-{timestr}"

...
###########Yuwei##########################
    ...
    @staticmethod
    def ts_inflation(delta=1e-3):
        return lambda summary: summary.radius_det(delta) /  np.sqrt(2*summary.d*np.log(2*summary.d/delta*2*summary.T))


    @staticmethod
    def ts_freq_inflation(delta=1e-3):
        return lambda summary: np.sqrt(2*summary.d*np.log(2*summary.d/delta*2*summary.T))
...
class TsWorthFunction(ProductWorthFunction):
    ...
    def update(self):
        ...
        self.radius_TS = self.inflation(self.summary)*self.radius_det
        ...
        self.compensator = (
            self.radius_TS  * basis.T @ (rand / scale ** 0.5)  /  np.sqrt(2*self.d*np.log(2*self.d/self.summary.delta*2*self.summary.T))
        )
...
class SgTsWorthFunction(WorthFunction):
    ...
    def compute(self, ctx: Context) -> np.ndarray:
        ...
        if alpha_approx > self.alpha:
            ...
            self.alphas.append(self.alpha_approx)
            #print(f"TS,self.alpha = {self.alpha}, alpha_approx = {alpha_approx}, values = {values}, uppers = {uppers}")
    ...
    def update(self):
        ...
        self.compensator = (
            #self.inflation(self.summary) * basis.T @ (rand / scale ** 0.5)
            self.radius_TS * basis.T @ (rand / scale ** 0.5)  /  np.sqrt(2*selfsummary.d*np.log(2*self.summary.d/self.summary.delta*2*self.summary.T))

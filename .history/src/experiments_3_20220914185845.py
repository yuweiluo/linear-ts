...
def run_experiments_k(
    ...
    outputs.to_csv(f"outputs2/all-unit-{n}-{d_min}-{d_max}-{d_gap}-{k}-{t}-{sim}-{prior_mu}-{prior_sd}-{noise_sd}-{thin_thresh}-{const_infl}-{gamma}-{timestr}.csv", index=False)


    outputs_last.to_csv(f"outputs2/all-unit-last-{n}-{d_min}-{d_max}-{d_gap}-{k}-{t}-{sim}-{prior_mu}-{prior_sd}-{noise_sd}-{thin_thresh}-{const_infl}-{gamma}-{timestr}.csv", index=False)

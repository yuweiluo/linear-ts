...
def run_experiments_k(
    ...
    outputs.to_csv(f"plots/all-{output_name}.csv", index=False)


    outputs_last.to_csv(f"plots/all-last-{output_name}.csv", index=False)
    ...
    for name, metric in metrics.items():
        ...
        plt.legend()
        plt.savefig(
            f"plots/{timestr}-{'last_k'}-{name}-{n}-{d_min}-{d_max}-{d_gap}-{k}-{t}-{sim}-{prior_mu}-{prior_sd}-{noise_sd}-{thin_thresh}-{const_infl}-{gamma}.pdf")

...
def run_experiments_d(
    ...
    plot_statistics(output_folder_name, output_name, figure_folder_name, mode = 'd', normalized = 1)
...
def run_experiments_gamma(
    ...
    for name, metric in metrics.items():
        ...
        for alg, agg in metric.items():
            ...
            for gamma in gamma_list:
                ...
                if name == 'errors':
                    plt.axhline(y=predicted_risk_[2], linestyle= '--', color='black', label='Predicted Risk')
        plt.xlabel("k")
        plt.ylabel(labels[name])
    ...
    plot_statistics(output_folder_name, output_name, figure_folder_name, mode = 'gamma', normalized=0,  x_lim = t)
    outputs.to_csv(f"{figure_folder_name}/all-{output_name}.csv", index=False)
    outputs_last.to_csv(f"{figure_folder_name}/all-last-{output_name}.csv", index=False)

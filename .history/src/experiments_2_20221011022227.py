...
def run_experiments_d(
    ...
    k_list = np.array([1,10,100, np.inf])
    ...
    for d in d_list:
        ...
        for k in k_list:
            ...
            alphas = defaultdict(MetricAggregator)
            ...
            for i in range(n):
                ...
                if sim == 1:  # Run Example 1
                    ...
                    if sim == 2:  # Run Example 2
                        print(f'Run Russo Scenario 2 for k = {k}')
                        results = russo_scenario2(
                            d=d, k=k, t=t, state_factory=state_factory,
                            prior_var=prior_sd ** 2/t, ## normalized prior variance
                            prior_mu=prior_mu,
                            noise_sd=noise_sd,
                            thin_thresh=thin_thresh,
                            const_infl=const_infl,
                        ...
                        if sim == 3:  # Run Russo Scenario for all methods
                            print('Run Inv Russo Scenario')
                            results = russo_scenario_inv(
                                d=d, k=k, t=t, state_factory=state_factory,
                                prior_var=prior_sd ** 2/t,
                                prior_mu=prior_mu,
                                noise_sd=noise_sd,
                ...
                for name, (regret, thinness, error, lambda_max, lambda_min, lambda_max_over_min, proj_first, alpha, approx_alpha, beta,zeta) in results.items():
            ...
            labels = {"regret": "Instantaneous Regret", "cumregret": "Cumulative Regret", "thinnesses": "Thinness", "errors": "Normalized Error", "lambda_maxs": "Maximal Eigenvalues", "lambda_mins": "Minimal Eigenvalues", "log_maxs_over_mins": "Log Max / Min", "lambdas_second": "Second Largest Eigenvalues", "lambdas_third": "Third Largest Eigenvalues", "biases": "Biases", "variances": "Variances", "risks": "Risks", "lambdas_d_minus_1": "d-1 Largest Eigenvalues", "lambdas_half_d": "d/2 Largest Eigenvalues", "projs_first": "Projection onto first eigenspace", "errors_candidate": "Normalized Error of Candidate", "errors_pcandidate": "Normalized Error of P-Candidate", "alphas": "Alpha", "betas": "Beta", "zetas": "Zeta", "lambda_maxs_over_mins": "Max / Min", "approx_alphas": "Approx Alpha"}
...
def run_experiments_gamma(
    n, gamma_min, gamma_max, gamma_gap, k, t, s, prior_mu=0.0, prior_sd=10.0, noise_sd=1.0,
    thin_thresh=2.0,
    const_infl=5.0,
    sim=0,
    gamma=1,
    ...
    #gamma_list = np.arange(gamma_min, gamma_gap + gamma_max, gamma_gap)
    gamma_list = np.array([0.1, 0.2, 0.5, 2.0, 5.0, 10.0])
    #k_list = np.array([1, 3, 10, 50, 100])
    k_list = np.array([1,10,100,1000,10000])

    outputs = []
    outputs_last = []
    for gamma in gamma_list:

        d = np.int64(np.ceil(t*gamma))
        #print(f" d: {d}, {type(d)}")
        ...
        for k in k_list:
            ...
            for i in range(n):
                ...
                print(f"Running experiment {i} for gamma {gamma} and k {k}...")
                if sim == 1:  # Run Example 1
                        print('Run Russo Scenario')
                        results = russo_scenario(
                            d=d, k=k, t=t, state_factory=state_factory,
                ...
                for name, (o_rerget, regret, thinness, error, lambda_max, lambda_min, lambda_second, lambda_third, bias, variance, risk, lambda_d_minus_1, lambda_half_d) in results.items():
                    o_regrets[name].add(o_rerget)
            ...
            labels = {"o_regret":"Oracle Instantaneous Regret",   "regret": "Instantaneous Regret", "cumregret": "Cumulative Regret", "thinnesses": "Thinness", "errors": "Normalized Error", "lambda_maxs": "Maximal Eigenvalues", "lambda_mins": "Minimal Eigenvalues", "log_maxs_over_mins": "Log Max / Min", "lambdas_second": "Second Largest Eigenvalues", "lambdas_third": "Third Largest Eigenvalues", "biases": "Biases", "variances": "Variances", "risks": "Risks", "lambdas_d_minus_1": "d-1 Largest Eigenvalues", "lambdas_half_d": "d/2 Largest Eigenvalues"}
    ...
    timestr = time.strftime("%Y%m%d-%H%M%S")
    output_name = f"gammas-t-{t}-radius-{radius}-prior_sd-{prior_sd}-noise_sd-{noise_sd}-sim-{sim}-k-{k}-prior_mu-{prior_mu}-n-{n}-gammas-{gamma_min}-{gamma_max}-{gamma_gap}-time-{timestr}"
    ...
    for name, metric in metrics.items():
        ...
        for alg, agg in metric.items():
            ...
            for gamma in gamma_list:
                ...
                output_ = outputs_last.loc[outputs_last['gamma'] == gamma]
                ...
                plt.fill_between(x, lower, upper, alpha=0.2)
                plt.plot(x, mean, label=alg+'_gamma_'+str(gamma))
                if name == 'errors':
                    predicted_risk_ = predicted_risk(gamma, radius, noise_sd)
                    plt.axhline(y=predicted_risk_[2], linestyle= '--', color='black')
    ...
    plot_statistics(output_folder_name, output_name, figure_folder_name, mode = 'gamma')
    outputs.to_csv(f"{figure_folder_name}/all-{output_name}.csv", index=False)
    outputs_last.to_csv(f"{figure_folder_name}/all-last-{output_name}.csv", index=False)
...
def __main__():
    ...
    if args.mode == 'd':
        ...
        run_experiments_gamma(n=args.n, gamma_min=args.para_min, gamma_max=args.para_max, gamma_gap=args.para_gap, k=args.k, t=args.t, s=args.s, prior_mu=args.pm, prior_sd=args.psd, noise_sd=args.nsd, thin_thresh=args.th, const_infl=args.inf, sim=args.sim, gamma=args.gamma, radius = args.radius)

...
class Roful(Policy):
    ...
    def calculate_oracle_alpha(self, feedback: Feedback):
        ...
        if expl[np.argmax(true_value).item()] == 0 and expl[feedback.arm_idx] == 0:
            ...
            oracle_alpha = expl[feedback.arm_idx] / \
                expl[np.argmax(true_value).item()]
    ...
    def update_metrics(self, feedback: Feedback):
        ...
        weight_G = (np.array(self.summary.basis) @
                    np.array(self.param))**2 / npl.norm(self.param)**2
        ...
        weight_diff = (beta / self.summary.scale **
                       0.5 * np.sign(weight_theta_hat))
...
class TsWorthFunction(ProductWorthFunction):
    ...
    def update(self):
        ...
        self.compensator = (
            self.radius_TS *
            basis.T @ (rand / scale ** 0.5) /
            self.summary.radius_normal(self.delta)
        )

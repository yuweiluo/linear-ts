...
def run_experiments_k(
    ...
    timestr = time.strftime("%Y%m%d-%H%M%S")
    output_name = f"n-{n}-dim_min-{d_min}-dim_max-{d_max}-d_gap-{d_gap}-k-{k}-t-{t}-sim-{sim}-prior_mu-{prior_mu}-prior_sd-{prior_sd}-noise_sd-{noise_sd}-thin_thresh-{thin_thresh}-const_infl-{const_infl}-gamma-{gamma}-radius-{radius}-time-{timestr}"

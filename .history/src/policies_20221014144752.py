...
###########Yuwei##########################
    ...
    def update_metrics(self, feedback: Feedback):
        ...
        beta = self.inflation(self.summary) * self.radius* np.sqrt(self.summary.lambda_) + self.noise_sd * np.sqrt(2 * (self.d/2)*np.log((self.summary.lambda_ + self.t))+2*  ( - self.d/2)*np.log( self.summary.lambda_ / self.delta  ))

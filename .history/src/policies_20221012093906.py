...
###########Yuwei##########################
    ...
    def update_metrics(self, feedback: Feedback):
        ...
        if zeta_square >= 0 and theta_norm_lower_bound>0:
            ...
            opt_new, wst_new = self.new_bound(self.summary.scale, np.maximum(self.x_XXnorm_min**2,lambda_min), np.minimum( self.x_XXnorm_max**2,lambda_max))
        ...
        if self.tt == self.t:
            ...
            print(f"newopt = {self.new_bound(lambda_max, lambda_min, self.x_XXnorm_min**2) }, opt_XX_inv_norm**2 = {opt_XX_inv_norm**2}")
            print(f"opt_new = {opt_new}, wst_new = {wst_new}")
    ...
    def new_bound (scale, XX_norm_sqr_min, XX_norm_sqr_max):
        ...
        lambda_up = np.min(scale[scale>=XX_norm_sqr])
        lambda_down = np.max(scale[scale<=XX_norm_sqr])

    ...
    for index in index_list:
        if index >= 0:
            ...
            plt.plot(x, mean, label=index_name+'='+str(index),
                     marker=markers[marker_index], markevery=50)
...
def plot_sample_path(df, x_name,  y_name, x_axis, y_axis, index_name_1, index_name_2,  save_path, gamma=None):
    ...
    for index_1 in index_list_1:
        if index_1 >= 0:
            ...
            plt.legend(loc='best', prop={'size': 6})
            plt.savefig(
                f"{save_path}-{index_name_1}-{str(index_1)}-2.jpg", dpi=600)
...
def plot_statistics(output_folder_name, output_name, figure_folder_name, mode,  gamma=None):
    ...
    attributes = ['errors', 'regret', 'cumregret', 'approx_alphas', 'oracle_alphas', 'worst_alphas',
                  'lambda_mins', 'lambda_maxs', 'projs_first', 'thinnesses',  'betas', 'zetas', 'betas_TS']
    ...
    for method in method_list:
        ...
        plot_last_iter(df, mode, method, mode, 'y', 'k',
                       f"{save_path}{method}-{output_name}'-1.jpg")

        df = pd.read_csv(f"{output_folder_name}/all-{output_name}.csv")
        plot_sample_path(df, mode, method, 't', 'y', mode, 'k',
                         f'{save_path}{method}-{output_name}', gamma)

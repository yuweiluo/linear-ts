...
def timing_block(name):
    # record the time of given the block using with statement
    # usage:
    '''
    with timing_block('name_of_block'):
        block to be timed
    '''
    start_time = time.time()
    yield
...
class DataSummary:
    ...
    def radius_det(self, delta=1e-3):
        if self._dirty:
            self._update_caches()
        term1 = np.log(self.scale / self.lambda_).sum() - \
            2 * np.log(delta/2/self.T)
    ...
    def calculate_alpha(self):
        ww, wst_x_XXinvnorm_sqr_TS = self.calculate_x_XXinvnorm_sqr_range(
            self.radius_TS())
        opt_x_XXinvnorm_sqr_RLS, oo = self.calculate_x_XXinvnorm_sqr_range(
            self.radius_det())
    ...
    def calculate_x_XXinvnorm_sqr_range(self,  ellipsoid_radius=None):
        ...
        if ellipsoid_radius == 0.0:
            theta_hat_Vt_inv_norm_sqr = theta_hat.T @ npl.solve(
                self.xx, theta_hat)
            ...
            if theta_hat_Vt_norm_sqr >= ellipsoid_radius**2:
                ...
                theta_norm_upper_bound = np.maximum(npl.norm(theta_hat - ellipsoid_radius/np.sqrt(
                    lambda_min) * self.basis[self.d - 1]), npl.norm(theta_hat + ellipsoid_radius/np.sqrt(lambda_min) * self.basis[self.d - 1]))
                theta_norm_upper_bound = npl.norm(
                    theta_hat) + ellipsoid_radius/np.sqrt(lambda_min)

                x_XXnorm_max = np.sqrt(
                    theta_hat.T @ self.xx @ self.xx @ theta_hat)/zeta
                x_XXnorm_min = zeta / theta_norm_upper_bound

                opt_x_XXinvnorm_sqr, wst_x_XXinvnorm_sqr = self.new_bound(self.scale, np.maximum(
                    x_XXnorm_min**2, lambda_min), np.minimum(x_XXnorm_max**2, lambda_max))
            else:
    ...
    def calculate_discrete_alpha(self, arms):
        lowers_CA, centers_CA, uppers_CA = self.confidence_bounds(
            arms, self.radius_TS())
        lowers_RLS, centers_RLS, uppers_RLS = self.confidence_bounds(
            arms, self.radius_det())
...
class MetricAggregator:
    ...
    def plot(self, ax, label, marker, mark_num, scale=2.0):
        ...
        ax.plot(x, mean, label=label, marker=marker,
...
class save_results:
    ...
    def save_outputs(self, output_folder_name, output_name):
        ...
        outputs.to_csv(
            f"{output_folder_name}/all-{output_name}.csv", index=False)
        outputs_last.to_csv(
            f"{output_folder_name}/all-last-{output_name}.csv", index=False)
...
def sample_ecdf(qe, pe=None, u=None, data_range=[0, 1]):
    ...
    result = (np.min(qe_array[pe_array >= u])
              if u <= max(pe) else data_range[1])

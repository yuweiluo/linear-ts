...
###########Yuwei##########################
    ...
    def calculate_oracle_alpha_inv(self, feedback : Feedback):
        ctx = feedback.ctx
        ...
        oracle_alpha_inv_ = expl[np.argmax(true_value).item()] / expl[feedback.arm_idx]

        
        return oracle_alpha_inv_
    ...
    def calculate_alpha(self):
        ...
        return np.sqrt(wst_x_XXinvnorm_sqr/opt_x_XXinvnorm_sqr) 
    ...
    def update_metrics(self, feedback: Feedback):
        ...
        self.approx_alpha_inv.append(self.calculate_alpha)
        oracle_alpha_inv = self.calculate_oracle_alpha_inv(feedback)
        print(f"oracle_alpha = {oracle_alpha_inv}")
        self.oracle_alpha.append(oracle_alpha_inv)


        self.outputs = ( self.approx_alpha, self.oracle_alpha, self.worst_alpha, self.metrics.regrets, self.thinnesses, self.errors, self.lambda_max, self.lambda_min,  self.proj_first,  self.betas, self.zeta)
...
class SgTsWorthFunction(WorthFunction):
    ...
    def compute(self, ctx: Context) -> np.ndarray:
        ...
        #print(f"SGTS,alpha = {self.alpha},alpha_approx = {alpha_approx},values = {values},uppers = {uppers}")
        #print(f"alpha_approx = {alpha_approx},alpha = {self.alpha}")
        if alpha_approx > self.alpha:
            ...
            #print(f"values = {values},final = {np.where(survivors, values, minus_infty)}")

...
###########Yuwei##########################
    ...
    def calculate_oracle_alpha(self, feedback : Feedback):
        ...
        print(f"expl = {expl}, chosen_arm = {feedback.chosen_arm},ture_index = {np.argmax(true_value).item()}")
        
        return expl[feedback.chosen_arm]/expl[np.argmax(true_value).item()]
    ...
    def update_metrics(self, feedback: Feedback):
        ...
        self.outputs = ( self.approx_alpha, self.worst_alpha, self.metrics.regrets, self.thinnesses, self.errors, self.lambda_max, self.lambda_min,  self.proj_first,  self.betas, self.zeta)

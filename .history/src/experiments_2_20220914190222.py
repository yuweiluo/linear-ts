...
def run_experiments_k(
    ...
    timestr = time.strftime("%Y%m%d-%H%M%S")
    output_name = f"dim_min-{d_min}-dim_max-{d_max}-d_gap-{d_gap}-prior_sd-{prior_sd}-noise_sd-{noise_sd}-gamma-{gamma}-radius-{radius}-sim-{sim}-k-{k}-t-{t}-prior_mu-{prior_mu}-thin_thresh-{thin_thresh}-const_infl-{const_infl}-n-{n}-time-{timestr}"
...
def __main__():
    ...
    if args.mode == 0:
        ...
        run_experiments_prior_sd(n=args.n, d_min=args.d_min, d_max=args.d_max, d_gap=args.d_gap, k=args.k, t=args.t, s=args.s, prior_mu=args.pm, prior_sd=args.psd, noise_sd=args.nsd, thin_thresh=args.th, const_infl=args.inf, sim=args.sim, gamma=args.gamma, radius = args.radius)

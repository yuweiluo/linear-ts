...
class Roful(Policy):
    ...
    def __init__(self, d, prior_var, worth_func: WorthFunction,  param=None, noise_sd = None, t = None, radius = None, delta = None):
        super().__init__()
    ...
    def ts(d, prior_var, inflation=1.0, state=None, param=None, noise_sd = None, t = None, radius = None, delta = None):
        ...
        return Roful(d, prior_var, TsWorthFunction(inflation, state=state), param, noise_sd = noise_sd, t = t, radius = radius, delta = delta)
...
###########Yuwei##########################
    ...
    def oful(d, alpha, radius):
        return Roful(d, alpha, SievedGreedyWorthFunction(radius, 1.0), param=None, noise_sd = None, t = None, radius = None, delta = None)
    ...
    def update_metrics(self, feedback: Feedback):
        ...
        if zeta_square >= 0 and theta_norm_lower_bound>0:
            ...
            self.x_XXnorm_min  =  zeta / theta_norm_upper_bound
        else:
        ...
        if zeta_square >= 0 and theta_norm_lower_bound>0:
            ...
            #opt_XX_inv_norm,opt_weight = self.solve_LP(self.summary.scale, self.summary.scale[9],self.summary.scale[8])
            ...
            self.approx_alpha.append(np.sqrt(self.x_XXinvnorm_min/opt_XX_inv_norm))
        ...
        if self.tt == self.t:
            ...
            print(f"lambda_min = {lambda_min},  self.x_XXnorm_min**2 = {self.x_XXnorm_min** 2},   self.x_XXnorm_max**2 = {self.x_XXnorm_max**2},lambda_max = {lambda_max} ")
            print(f"1/sqrt_scale = {1/np.sqrt(self.summary.scale)}")
            print(f"sqrt_scale = {np.sqrt(self.summary.scale)}")
            print(f"np.maximum(self.x_XXnorm_min**2,lambda_min) = {np.maximum(self.x_XXnorm_min**2,lambda_min)}")
            print(f"newopt = {self.new_bound(lambda_max, lambda_min, np.maximum(self.x_XXnorm_min**2,lambda_min)) }, opt_XX_norm! = {np.sqrt(theta_opt.T @ self.summary.xx @ theta_opt/ npl.norm(theta_opt)**2)}")
        ...
        self.outputs = ( self.oracle_regrets, self.metrics.regrets, self.thinnesses, self.errors, self.lambda_max, self.lambda_min, self.lambda_max_over_min, self.proj_first, self.alpha,self.approx_alpha, self.beta, self.zeta)
    
    @staticmethod
    def new_bound (lambda_max, lambda_min, XX_norm_sqr):
        return 1/lambda_max + 1/lambda_min - XX_norm_sqr/lambda_min/lambda_min
...
class GradTsWorthFunction(ProductWorthFunction):
    compensator: np.ndarray

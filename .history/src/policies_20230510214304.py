...
class TsWorthFunction(ProductWorthFunction):
    ...
    def update(self):
        ...
        self.compensator = (
            self.radius_TS  * basis.T @ (rand / scale ** 0.5)  /  np.sqrt(2*self.d*np.log(2*self.d/self.delta*2*self.T))
        )
...
class SgTsWorthFunction(WorthFunction):
    ...
    def update(self):
        ...
        self.compensator = (
            #self.inflation(self.summary) * basis.T @ (rand / scale ** 0.5)
            self.inflation(self.summary) * basis.T @ (rand / scale ** 0.5)  /  np.sqrt(2*self.d*np.log(2*self.d/self.delta*2*self.T))

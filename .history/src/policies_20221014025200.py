...
class Roful(Policy):
    ...
    @staticmethod
    # added param as input to calculate error
    def spects(d, prior_var, inflation=1.0, state=None, param=None, noise_sd = None, t = None, radius = None, delta = None, alpha = None, radius_oful = None, tolerance = None):
        if isinstance(inflation, float):
            inflation = Roful.const_inflation(inflation)

        return Roful(d, prior_var, SgTsWorthFunction(inflation, alpha = alpha, radius = radius, noise_sd = noise_sd, delta = delta,radius_oful = radius_oful, tolerance = tolerance, state=state), param, noise_sd = noise_sd, t = t,  radius = radius, delta = delta, inflation = inflation)
...
class SgTsWorthFunction(WorthFunction):
    ...
    def compute(self, ctx: Context) -> np.ndarray:
        ...
        if self.calculate_alpha() > self.alpha:
            return values
        else:
            return uppers
    ...
    def calculate_alpha(self, beta):
        ...
        if theta_hat_Vt_norm_sqr >= beta**2:
            zeta = (theta_hat_Vt_norm_sqr - beta**2)**0.5
            #theta_norm_lower_bound = npl.norm(theta_hat) - beta/np.sqrt()
            theta_norm_upper_bound = np.maximum( npl.norm(theta_hat - beta/np.sqrt(lambda_min) * self.summary.basis[self.summary.d - 1]),npl.norm(theta_hat + beta/np.sqrt(lambda_min) * self.summary.basis[self.summary.d - 1]))
            x_XXnorm_max  =  np.sqrt(theta_hat.T @ self.summary.xx @ self.summary.xx @ theta_hat)/zeta
            x_XXnorm_min  =  zeta / theta_norm_upper_bound

            opt_new, wst_new = self.new_bound(self.summary.scale, np.maximum(x_XXnorm_min**2,lambda_min), np.minimum( x_XXnorm_max**2,lambda_max))
            
        return wst_new/opt_new

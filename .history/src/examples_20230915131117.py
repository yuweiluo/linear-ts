...
def emp_scenario(
    ...
    env = RealDatasetEnvironment(
        ctx_gen, label_gen, labels.unique(), noise_gen)
...
def russo_scenario(state_factory, k=1, d=100, t=1000, prior_var=2.0, arm_bound=0.1, prior_mu=0.0, noise_sd=1.0, thin_thresh=2.0, const_infl=5.0, radius=1.0, alpha=0.5):
    ...
    ctx_gen = CtxGenerator.uniform_on_sphere(k, d,  np.sqrt(
        d), state=state_factory())  # variance of d-unit sphere is 1/d

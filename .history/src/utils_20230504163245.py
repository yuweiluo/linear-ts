...
class DataSummary:
    ...
    def __init__(self, dim, lambda_, noise_sd, param_bound, param, inflation):
    ...
    def radius_TS(self, delta=1e-4 ):
        self.inflation(self) * np.sqrt(2*self.d*np.log(2*self.d/self.delta*2*self.t)
    ...
    def calculate_alpha(self):
        _, wst_x_XXinvnorm_sqr_TS = self.calculate_x_XXinvnorm_sqr_range(self.inflation(self))
        opt_x_XXinvnorm_sqr_RLS, _ = self.calculate_x_XXinvnorm_sqr_range(1.0)
    ...
    def calculate_x_XXinvnorm_sqr_range(self, inflation):
        theta_hat =  self.mean
        theta_hat_Vt_norm_sqr = theta_hat.T @ self.xx @ theta_hat
        lambda_max = max(self.scale)
        lambda_min = min(self.scale)
        
        self.beta_TS = inflation * self.radius_det()
        if theta_hat_Vt_norm_sqr >= self.beta_TS**2:
            
            zeta = (theta_hat_Vt_norm_sqr - self.beta_TS**2)**0.5
            #theta_norm_lower_bound = npl.norm(theta_hat) - beta/np.sqrt()
            theta_norm_upper_bound = np.maximum( npl.norm(theta_hat - self.beta_TS/np.sqrt(lambda_min) * self.basis[self.d - 1]),npl.norm(theta_hat + self.beta_TS/np.sqrt(lambda_min) * self.basis[self.d - 1]))
            theta_norm_upper_bound = npl.norm(theta_hat) + self.beta_TS/np.sqrt(lambda_min)

...
class DataSummary:
    ...
    def radius_TS(self, delta=1e-3 ):
        ...
        if self._dirty:
            self._update_caches()
        print(f"self.T = {self.T}, delta = {delta}, inflation = {self.inflation(self)}, radius_TS = {self.inflation(self)*self.radius_det(delta = delta)}, radius_normal = { np.sqrt(2*self.d*np.log(2*self.d/delta*2*self.T))}")

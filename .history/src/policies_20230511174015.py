...
###########Yuwei##########################
    ...
    def ts_inflation_alter(delta=1e-3):
        # print(self.summary.noise_sd * self.summary.radius_normal(1e-3) / self.summary.radius_det(1e-3) )
    ...
    def update_metrics(self, feedback: Feedback):
        ...
        #print(f"beta_TS = {beta_TS}, beta = {self.beta},self.summary.radius_TS() = {self.summary.radius_TS()}, self.summary.radius_det() = {self.summary.radius_det()}")
        assert((self.beta+beta_TS)/self.beta == (self.summary.radius_TS()+self.summary.radius_det())/self.summary.radius_det())

        #self.oracle_alpha.append(self.worth_func.radius_det/(self.worth_func.radius_det+self.worth_func.radius_TS)*self.worth_func.alpha_approx)
        ...
        #print(f"ts_inflation_alter = {self.summary.noise_sd * self.summary.radius_normal(1e-3) / self.summary.radius_det(1e-3)} " )
        
        self.outputs = ( self.worth_func.alphas, self.worth_func.mus, self.worst_alpha, self.metrics.regrets, self.thinnesses, self.errors, self.lambda_max, self.lambda_min,  self.proj_first,  self.betas, self.betas_TS)
...
class SgTsWorthFunction(WorthFunction):
    ...
    def compute(self, ctx: Context) -> np.ndarray:
        ...
        #assert(alpha_approx<=self.alpha)
        if alpha_approx > self.alpha:
            self.alpha_approx = alpha_approx
            self.alphas.append(self.alpha_approx)
            self.mu_approx = mu_approx
            self.mus.append (self.mu_approx)
            #print(f"TS,self.alpha = {self.alpha}, alpha_approx = {alpha_approx}, values = {values}, uppers = {uppers}, mu_approx = {mu_approx}")

            return values
        else:
            self.alpha_approx = alpha_approx
            #print("oful!")
            #print(f"alpha_approx={self.alpha_approx}")
            ...
            #print(f"oful_ratio={(self.summary.radius_TS()+self.summary.radius_det())/self.summary.radius_TS()},self.alpha = {self.alpha}, alpha_approx = {alpha_approx} ")
            #print(f"radius_TS = {self.summary.radius_TS()},radius_det = {self.summary.radius_det()}")
            #print(f"oful,self.alpha = {self.alpha},alpha_approx = {alpha_approx}, values = {values}, uppers = {uppers}")
            return uppers

...
###########Yuwei##########################
    ...
    def update_metrics(self, feedback: Feedback):
        ...
        self.oracle_alpha.append(beta_TS/(beta+beta_TS)*self.summary.calculate_alpha)
...
class TsWorthFunction(ProductWorthFunction):
    ...
    def compute(self, ctx: Context) -> np.ndarray:
        ...
        self.alphas.append(self.summary.calculate_alpha)
...
class SievedGreedyWorthFunction(WorthFunction):
    ...
    def compute(self, ctx: Context) -> np.ndarray:
        ...
        if self.tolerance == 1.0:
            self.alphas.append(            self.alphas.append((self.summary.radius_TS()+self.summary.radius_det())/self.summary.radius_TS())
)
        else:
            self.alphas.append(self.summary.calculate_alpha*            self.alphas.append((self.summary.radius_TS()+self.summary.radius_det())/self.summary.radius_TS())
)
...
class SgTsWorthFunction(WorthFunction):
    ...
    def compute(self, ctx: Context) -> np.ndarray:
        ...
        if alpha_approx > self.alpha:
            ...
            self.alphas.append((self.summary.radius_TS()+self.summary.radius_det())/self.summary.radius_TS())
            return uppers

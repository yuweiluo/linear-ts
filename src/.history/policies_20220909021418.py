...
class Roful(Policy):
    ...
    def ts(d, prior_var, inflation=1.0, state=None, param=None):
        if isinstance(inflation, float):
            inflation = Roful.const_inflation(inflation)

        return Roful(d, prior_var, TsWorthFunction(inflation, state=state), param)
...
###########Yuwei##########################
    ...
    def update_metrics(self, feedback: Feedback):
        ...
        def func(x):
            ...
            return 1-1/self.gamma -  self.discrete_integral(func_,  weight_H, self.summary.scale)

        c_0 = fsolve(func, [0,20])

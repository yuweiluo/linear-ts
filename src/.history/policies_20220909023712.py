...
###########Yuwei##########################
    ...
    def update_metrics(self, feedback: Feedback):
        ...
        int_H_1 = self.discrete_integral(func1_, weight_H, self.summary.scale)
        int_H_2 = self.discrete_integral(func2_, weight_H, self.summary.scale)
        int_G_1 = self.discrete_integral(func1_, weight_G, self.summary.scale)

        B_ = npl.norm(self.param)**2 * (1+ gamma * c_0 *int_H_2/ int_H_1 ) * int_G_1
        V_ = npl.norm(self.param)**2 * (1+ gamma * c_0 *int_H_2/ int_H_1 ) * int_H_1

        self.B_n.append(B_)
        self.V_n.append(V_)
        self.R_n.append(B_ + V_)
        print(f"t = {self.t}, B_n = {B_}, V_n = {V_}, R_n = {B_ + V_}")


        

        self.outputs = ( self.metrics.regrets, self.thinnesses, self.errors, self.lambda_max, self.lambda_min, self.lambda_second, self.lambda_third,self.B_n, self.V_n, self.R_n)

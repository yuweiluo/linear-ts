...
def run_experiments_prior_sd(n, d_min, d_max, d_gap, k, t, s, prior_mu=0.0, prior_sd=10.0, noise_sd=1.0, thin_thresh=2.0, const_infl=5.0, sim=0, gamma=1):
    ...
    outputs.to_csv(f"plots/all-{n}-{d_min}-{d_max}-{d_gap}-{k}-{t}-{sim}-{prior_mu}-{prior_sd}-{noise_sd}-{thin_thresh}-{const_infl}-{gamma}.csv", index=False)


    outputs_last.to_csv(f"plots/all-last-{n}-{d_min}-{d_max}-{d_gap}-{k}-{t}-{sim}-{prior_mu}-{prior_sd}-{noise_sd}-{thin_thresh}-{const_infl}-{gamma}.csv", index=False)
    ...
    for name, metric in metrics.items():
        ...
        plt.legend()
        plt.savefig(f"plots/{name}-last-{n}-{d_min}-{d_max}-{d_gap}-{k}-{t}-{sim}-{prior_mu}-{prior_sd}-{noise_sd}-{thin_thresh}-{const_infl}-{gamma}.pdf")
...
def run_experiments_k(
    ...
    outputs.to_csv(
        f"plots/{'all'}-{n}-{d_min}-{d_max}-{d_gap}-{k}-{t}-{sim}-{prior_mu}-{prior_sd}-{noise_sd}-{thin_thresh}-{const_infl}-{gamma}.csv", index=False)
    outputs_last.to_csv(
        f"plots/{'all'}-{'last'}-{n}-{d_min}-{d_max}-{d_gap}-{k}-{t}-{sim}-{prior_mu}-{prior_sd}-{noise_sd}-{thin_thresh}-{const_infl}-{gamma}.csv", index=False)
    ...
    for name, metric in metrics.items():
        ...
        plt.legend()
        plt.savefig(
            f"plots/{name}-{'last'}-{n}-{d_min}-{d_max}-{d_gap}-{k}-{t}-{sim}-{prior_mu}-{prior_sd}-{noise_sd}-{thin_thresh}-{const_infl}-{gamma}.pdf")
